...
import { CheckCircle, LibraryBooks, Undo } from '@mui/icons-material';
...
export const ReshelveItemPage: FC = () => {
  ...
  const { data: returned_items = [], isLoading: loading } = useCopiesUnshelved(
    selected_branch?.id || 1
  );

  const reshelve_mutation = useReshelveCopy({
    skipInvalidation: true, // Don't invalidate unshelved_item_copies - we manage state manually
    onSuccess: () => {
      // Success handling is done in handle_reshelve
    },
    onError: () => {
      // Error handling is done in handle_reshelve
    },
  });

  const undo_reshelve_mutation = useUndoReshelve({
    onSuccess: () => {
      set_success('Reshelve undone successfully!');
      set_error(null);
    },
    onError: (error) => {
      set_error(error.message || 'Failed to undo reshelve');
      set_success(null);
    },
  });

  const handle_reshelve = (copy_id: number) => {
    // Find the item before reshelving
    const item_to_reshelve = returned_items.find((item) => item.id === copy_id);

    if (!item_to_reshelve) {
      set_error('Item not found');
      return;
    }
    ...
    // Add the item to recently_reshelved_items IMMEDIATELY using functional update
    // This ensures it's in state before the mutation runs
    ...
    reshelve_mutation.mutate(
      { copy_id, branch_id: selected_branch?.id },
      {
        onSuccess: () => {
          set_success('Item reshelved successfully!');
          set_error(null);
          // Don't update the cache - let recently_reshelved_items handle the display
          // The item is already in recently_reshelved_items with status "Available"
        },
        onError: (error) => {
          set_error(error.message || 'Failed to reshelve item');
          set_success(null);
          // If reshelve fails, remove it from recently_reshelved_items
          set_recently_reshelved_items((prev) =>
  ...
  // Merge returned items with recently reshelved items for display
  // Recently reshelved items take precedence (they have status "Available" and show undo button)
  // Filter out items from returned_items that are in recently_reshelved_items
  const all_items = [
    ...recently_reshelved_items,
    ...returned_items.filter(
      (item) =>
  ...
  const scan_and_identify = async () => {
    ...
    try {
      ...
      // Calculate late fee ($1.00 per day)
      const late_fee = days_overdue * 1.0;
  ...
  return (
    <PageContainer>
      <PageTitle
        title={'Reshelve / Check-In Item'}
        Icon_Component={LibraryBooks}
      />
                ...
                <Table stickyHeader>
                  ...
                  <TableBody>
                    {all_items.map((item) => (
                      ...
                      >
                        <TableCell>{item.id || `Copy ${item.id}`}</TableCell>
                        <TableCell>{item.library_item_title}</TableCell>
                        <TableCell>
                          <Chip
                        ...
                        <TableCell align="right">
                          ...
                          (item.status &&
                            ...
                            <Button
                              variant="contained"
                              size="small"
                              onClick={() => handle_reshelve(item.id)}
                              disabled={reshelve_mutation.isPending || loading}
                              startIcon={
                                reshelve_mutation.isPending ? (
            ...
            >
              ...
              {transaction_info.is_overdue && !is_damaged ? (
                ...
                >
                  ...
                  >
                    ($1.00 per day) - will be applied to patron account
                  </Typography>
                </Box>
              ) : !is_damaged ? (
                <Box
