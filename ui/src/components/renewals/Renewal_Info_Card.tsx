...
interface View_Reservations_Drawer_Props {
  open: boolean;
  on_close: () => void;
...
function View_Reservations_Drawer({
  open,
  on_close,
  reservations,
  is_loading,
  item_title,
}: View_Reservations_Drawer_Props) {
  ...
  return (
    <Drawer anchor='right' open={open} onClose={on_close}>
      <Box sx={{ width: 400, p: 2, mt: '64px', maxWidth: '100vw' }}>
        <Stack
          direction='row'
        ...
        >
          ...
          <IconButton onClick={on_close} size='small'>
            <Close />
          </IconButton>
        </Stack>
        ...
        {is_loading && (
          <CircularProgress
            size={'clamp(48px, calc(36px + 2.5vw), 96px)'}
            sx={{
              display: 'block',
              mx: 'auto',
              width: { xs: '24px', sm: '48px', md: '64px', lg: '96px' },
              height: { xs: '24px', sm: '48px', md: '64px', lg: '96px' },
            }}
          />
        )}

        {!is_loading && (!reservations || reservations.length === 0) && (
          <Alert severity='info'>No reservations found for this item.</Alert>
        )}
              ...
              >
                ...
                >
                  <ListItemAvatar>
                    <Avatar src={reservation.patron_image || undefined}>
                      {reservation.first_name?.[0]}
                      {reservation.last_name?.[0]}
                    </Avatar>
                  </ListItemAvatar>
                  <ListItemText
                    primary={
                      ...
                      >
                        ...
                        <Chip
                          label={`#${reservation.queue_position}`}
                          size='small'
                          variant='outlined'
                          sx={{ fontWeight: 'bold' }}
                        />
                ...
                >
                  <Chip
                    ...
                    size='small'
                    color={get_status_color(reservation.status)}
                    icon={get_status_icon(reservation.status) || undefined}
                  />
                  <Typography variant='caption' color='text.secondary'>
                    Reserved:{' '}
                    {dayjs(reservation.reservation_date).format('MMM D, YYYY')}
                  </Typography>
                  {reservation.expiry_date && (
                    <Typography variant='caption' color='text.secondary'>
                      Expires:{' '}
                      {dayjs(reservation.expiry_date).format('MMM D, YYYY')}
                    </Typography>
                  )}
                </Stack>
              </ListItem>
...
export function Renewal_Info_Card({
  ...
  const { data: reservations, isLoading } = useReservationsByItemCopy(
    selected_item.id,
  );

  const handle_open_drawer = () => set_drawer_open(true);
  const handle_close_drawer = () => set_drawer_open(false);

  return (
    <>
      <View_Reservations_Drawer
        open={drawer_open}
          ...
          <Stack spacing={3}>
            ...
            {eligibility.can_renew && eligibility.is_overdue && (
              ...
              >
                ...
                This item is overdue. Renewing will set a new due date from
                today, but any accrued fines may still apply.
              </Alert>
            )}
