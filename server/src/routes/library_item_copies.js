  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        ROW_NUMBER() OVER (PARTITION BY lic.library_item_id ORDER BY lic.id) as copy_number,
        COUNT(*) OVER (PARTITION BY lic.library_item_id) as total_copies,
		COUNT(r.id) OVER (PARTITION BY r.item_copy_id) as reservation_count
      FROM LIBRARY_ITEM_COPIES lic
      JOIN LIBRARY_ITEMS ci ON lic.library_item_id = ci.id
	  LEFT JOIN RESERVATIONS r ON lic.id = r.item_copy_id AND r.status IN ('ready', 'waiting')
      LEFT JOIN BRANCHES b ON lic.current_branch_id = b.id
      LEFT JOIN BRANCHES bb ON lic.owning_branch_id = bb.id
      LEFT JOIN PATRONS p ON lic.checked_out_by = p.id
      LEFT JOIN BOOKS bk ON ci.id = bk.library_item_id
      LEFT JOIN VIDEOS v ON ci.id = v.library_item_id
      LEFT JOIN AUDIOBOOKS ab ON ci.id = ab.library_item_id
      LEFT JOIN VINYL_ALBUMS va ON ci.id = va.library_item_id
      LEFT JOIN CDS cd ON ci.id = cd.library_item_id
      ${conditions}
      ORDER BY ci.title, lic.id
    `;
    ...
    // Add copy labels using pre-calculated numbers from SQL
    const copies_with_labels = item_copies.map((copy) => {
      const label = `Copy ${copy.copy_number} of ${copy.total_copies}`;
      let cover = null;
      ...
      // Clean up the temporary cover image fields
      delete x.book_cover_image;
      delete x.audiobook_cover_image;
      delete x.video_cover_image;
      delete x.vinyl_cover_image;
      delete x.cd_cover_image;
...
router.get('/checked-out-simple', async (req, res) => {
  try {
    ...
    const query = `
      SELECT 
	  	lic.id,
        lic.current_branch_id,
		lic.condition,
        ci.title,
        ci.item_type,
		CASE
            WHEN lic.due_date < ${now} THEN 1
            ELSE 0
		END as is_overdue
      FROM LIBRARY_ITEM_COPIES lic
      JOIN LIBRARY_ITEMS ci ON lic.library_item_id = ci.id
      ${conditions}
      ORDER BY lic.id, ci.title
    `;
  ...
  try {
    ...
    const query = `
      ...
      FROM LIBRARY_ITEM_COPIES ic
        ...
        JOIN ITEM_TRANSACTIONS t ON ic.id = t.item_copy_id
      WHERE 
