...
router.get('/checkouts', (req, res) => {
  ...
  let query = `SELECT 
               ...
               JOIN LIBRARY_ITEM_COPIES lic ON t.item_copy_id = lic.id
...
router.get('/fines', (req, res) => {
  ...
  let query = `SELECT 
               ...
               JOIN ITEM_TRANSACTIONS t ON f.transaction_id = t.id
               JOIN LIBRARY_ITEM_COPIES lic ON t.item_copy_id = lic.id
               JOIN LIBRARY_ITEMS li ON lic.library_item_id = li.id
               WHERE 1=1`;
  const params = [];
                ...
                .then((damaged_items) => {
                  ...
                  return db
                    ...
                    .then((total_patrons) => {
                      ...
                      return db
                        .execute_query(
                          'SELECT COUNT(*) as count FROM ITEM_TRANSACTIONS'
                        )
...
router.get('/stats/overview', (_, res) => {
  ...
  var optimized_query = `
	...
	transaction_stats AS (
		SELECT 
			COUNT(*) as active_checkouts,
			SUM(CASE WHEN lic.due_date < CURRENT_DATE THEN 1 ELSE 0 END) as overdue_items
		FROM LIBRARY_ITEM_COPIES lic
		WHERE lic.checked_out_by IS NOT NULL
	)
    SELECT 
      ls.total_items,
      ls.available_items,
