  ...
  try {
    ...
    const transaction = await db.execute_query(
      `SELECT * FROM TRANSACTIONS 
       WHERE copy_id = ? AND status = 'Active' AND UPPER(transaction_type) = 'CHECKOUT' 
       LIMIT 1`,
      [copy_id]
    );
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        END as days_overdue
      FROM TRANSACTIONS t
      INNER JOIN LIBRARY_ITEM_COPIES ic ON t.copy_id = ic.id
  ...
  async (req, res) => {
    try {
      ...
      const active_checkout_count = await db.execute_query(
        'SELECT COUNT(*) as count FROM TRANSACTIONS WHERE patron_id = ? AND UPPER(status) = "ACTIVE" AND UPPER(transaction_type) = "CHECKOUT"',
        [patron_id]
      );
      ...
      const query = `
        ...
        WHERE t.copy_id = ? AND t.patron_id = ? AND t.status = 'Active' AND t.transaction_type = 'checkout'
  ...
  async (req, res) => {
    try {
      ...
      const check_out_transaction = await db.execute_query(
        'SELECT * FROM TRANSACTIONS WHERE copy_id = ? AND status = "Active" AND UPPER(transaction_type) = "CHECKOUT" LIMIT 1',
        [copy_id]
      );
...
router.put('/:id/renew', async (req, res) => {
  try {
    ...
    // Update the original checkout transaction
    ...
    await db.create_record('TRANSACTIONS', renewal_transaction);
...
router.get('/by-copy/:copy_id', async (req, res) => {
  try {
    ...
    const transactions = await db.execute_query(
      'SELECT * FROM TRANSACTIONS WHERE copy_id = ? AND status = "Active" AND transaction_type = "checkout" ORDER BY created_at DESC LIMIT 1',
      [copy_id]
    );
